...
def write_to_db(events_df: pd.DataFrame, gt_df: pd.DataFrame) -> None:
    ...
    try:
        ...
        try:
            ...
            # #3) — drop it here if it's a leftover from before that split,
            # so an ops.duckdb generated by older code doesn't keep an
            # audit-log table sitting inside the file the guardrail is
            # supposed to make it physically unreachable from (Epic 5
            # review round 2, #4: the committed file still had one).
            init_schema(conn)
